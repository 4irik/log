...
Как видим, у нас выполнились оба теста несмотря на то, что первый тест должен был бы прекратить выполнение скрипта вызовом `exit 1` но это не произошло потому что его мы запустили в отдельном интерпретаторе (`sh ./$script_file`).
